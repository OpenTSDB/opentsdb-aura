...
/**
 * This is a flusher that will take a shard, the clone of a shard's hash table
 * and a set of one or more stores to write to and flushes each segment found
 * for the base time.
 */
public class TSFlusherImp implements TSFlusher {
                        ...
                        for (int i = 0; i < flushInfos.length; i++) {
                            ...
                            synchronized (flushInfos) {
                                ...
                                if (ts + (int) ((frequency / 1000) * 2) < System.currentTimeMillis() / 1000) {
                                    LOGGER.warn("Flush for shard " + i + "@" + fi.baseTimestamp + " has been running for "
                                            + ((int) (System.currentTimeMillis() / 1000) - ts) + " so cancelling.");
                                    //fi.cancel();
                                }
                            }
    ...
    public FlushStatus flushShard(final int shardId,
        ...
        synchronized (flushInfos) {
            if (flushInfos[shardId] == null) {
                ...
                LOGGER.error("Failed to start flush for shard {}@{} as a flush is still ongoing.", shardId, flushTimestamp);
            }
        }
        return flushStatus;
    }
    ...
    /**
     * Instantiated for each flush at each timestamp. This holds the jobs (a single
     * thread) so we can run multiple threads if we need to.
     */
    class FlushInfo {
        private final String[] tagSet;
        ...
        void complete() {
            LOGGER.debug("Finishing shard flush for " + shardId + "@" + baseTimestamp + " with " + flushed + " records");
            flushedGuage.set(flushed, tagSet);
            oooGuage.set(ooo, tagSet);
            failureGuage.set(failures, tagSet);
            emptyGuage.set(noSegment, tagSet);
        ...
        class Job extends Thread {
            ...
            private final TimeSeriesEncoder encoder;
            private int flushed;
            private int ooo;
            private int failures;
            private int noSegment;
                ...
                while (true) {
                    try {
                        ...
                        if (addr == 0) {
                            ...
                            synchronized (jobs) {
                                ...
                                if (nulls == jobs.length) {
                                    complete();
                                    LOGGER.info("Finished flushing segments for shard {}@{}", flushed, shardId, baseTimestamp);
                                    return;
                                }
                            }
                        ...
                        if (encoder.segmentHasOutOfOrderOrDuplicates()) {
                            TimeSeriesEncoder reWrite = encoders.get();
                            try {
                                ...
                                LOGGER.error("Failed to flush an OOO segment for {}@{}", shardId, baseTimestamp, e);
                                failures++;
                            } finally {
                                // CRITICAL!
                                reWrite.freeSegment();
